Implementation:
    - Attempted the use of op-codes and arrays/registers as speficied in the assginment using C
    - Initialized a 2D array to store all program array with a set size of 50,000 (not the most memory efficient)
    - Initialized the first array in the 2D array as the zero-array with a set size of 15,000
    - When an array is freed, there is an indicator for that free array. If there is no such free array, the program iterates through all arrays in the 2D array until it comes across an empty array(not the most time efficient)
    - Wrote everything in the main function to avoid time delays when calling functions
    - Uses a 1D array for the 8 registers and a 1D array to store the length of each 1D array in the 2D array (helpful for checking which ones are free) 
    - Attempted to create new logic for finding an empty array that is quicker by utilizing an array that stores all empty array indices. This proved to have too many error
    - Looked into gcc flags that made program run almost 50% quicker

To compile and run: 
    - gcc -Ofast -march=native warmup3.c or gcc -Ofast -march=native -o file-name warmup3.c
    - ./a.out executable-name.um or ./file-name executable-name.um

Disassembler:
- To run:
    - gcc disassembler.c or gcc -o file-name disassembler.c
    - ./a.out input.um output.uma 

Assembler:
- To run:
    - gcc assembler.c or gcc -o file-name disassembler.c
    - ./a.out input.uma output.um

Timings:
- sandmark.um : 15.131 s



Sandmark Output:

yshu@b146-46:~/cs454/warmup3$ date
Sun Sep  7 08:56:23 PM MDT 2025
yshu@b146-46:~/cs454/warmup3$ gcc -Ofast -march=native warmup3.c
yshu@b146-46:~/cs454/warmup3$ time ./a.out ../sandmark.um
